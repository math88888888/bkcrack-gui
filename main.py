        ...
        try:
            with zipfile.ZipFile(self.compressedZipPath, 'r') as zip_ref:
                ...
                for file in file_list:
                    ...
                    print("正在处理文件:", file, "=>", temp_path)  # 调试输出
                    ...
                    if key:
                        ...
                        if len(key_parts) == 3:
                            ...
                            print("解密命令输出:", result.stdout)  # 调试输出
                            if result.returncode != 0:
                                print("解密失败:", result.stderr)  # 调试输出
                                continue
                    ...
                    if os.path.exists(temp_path):
                        ...
                        print("文件提取成功:", temp_path)  # 调试输出
                    else:
                        print("文件提取失败:", temp_path)  # 调试输出

                if not temp_files:
                    raise Exception("没有成功提取任何文件")

                print("最终可预览文件列表:", temp_files)  # 调试输出
...
if __name__ == "__main__":
    ...
    window.setWindowTitle("bkcrack-gui v0.91  Author: 星辰不及阁下")
    window.show()
    sys.exit(app.exec())
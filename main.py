...
class MainWindow(QWidget, Ui_Form):
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                if file_list:
                    ...
                    if file_list:
                        self.append_colored_output(f"已自动填充目标文件列表，当前选择: {file_list[0]}", QColor("yellow"))
        except Exception as e:
    ...
    def GetCompressedZipInfo(self):
        ...
        try:
            ...
            self.append_colored_output("\n压缩包文件源数据区部分信息:", QColor("magenta"))
            self.append_colored_output(creator_info, QColor("magenta"))
        except Exception as e:
            self.append_colored_output(f"\n无法获取压缩包创建者信息: {str(e)}", QColor("red"))
    ...
    def execute_hex_command(self):
        ...
        if not all([target_file, hex_offset, hex_pattern]):
            self.append_colored_output("请确保填写了目标文件(-c)、十六进制偏移量和字节模式", QColor("red"))
            return
    ...
    def direct_hex_attack(self):
        ...
        if not hex_offset or not hex_pattern:
            self.append_colored_output("请填写十六进制偏移量和字节模式", QColor("red"))
            return

        # 构建命令
        command = ["bkcrack.exe", "-C", self.compressedZipPath, "-c", target_file,
                   "-x", hex_offset, hex_pattern]

        self.OutPutArea.clear()
        self.append_colored_output("正在执行直接十六进制攻击命令: " + " ".join(command), QColor("yellow"))
        self.append_colored_output("正在进行攻击，请稍等...", QColor("yellow"))
...
if __name__ == "__main__":
    ...
    window.setWindowTitle("bkcrack GUI v0.8  Author: 星辰不及阁下")
    window.show()
    sys.exit(app.exec())
...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        direct_hex_group = QGroupBox("(没有-P,-p的时候）只有(-x) , 支持多个参数内容(以;分割)")
        direct_hex_layout = QVBoxLayout(direct_hex_group)
        # 新增输入转16进制功能 - 放在direct_hex_group内的第一行
        hex_conversion_layout = QHBoxLayout()
        self.HexConversionInput = PlainTextEdit()
        self.HexConversionInput.setMinimumHeight(35)
        hex_conversion_layout.addWidget(self.HexConversionInput)

        self.ConvertToHexButton = QPushButton("转16进制")
        self.ConvertToHexButton.setProperty("execButton", True)
        self.ConvertToHexButton.setMinimumHeight(35)
        hex_conversion_layout.addWidget(self.ConvertToHexButton)
        direct_hex_layout.addLayout(hex_conversion_layout)
    ...
    def retranslateUi(self, Form):
        Form.setWindowTitle("bkcrack-gui v0.92   Author: 星辰不及阁下")
